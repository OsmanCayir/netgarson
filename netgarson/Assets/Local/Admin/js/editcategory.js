$("#button-deleteImage").click(function () {
    $.ajax({
        ...
        url: "/Admin/DeleteCategoryImageBeforeUpdate",
...
function loadImageUploader(){
    var extraObj = $("#fileuploader").uploadFile({
        ...
        deleteCallback: function (data, pd) {
            $.post("/Admin/DeleteCategoryImage?url=" + data.url,
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == 100) {
                ...
                toastr.error("Kategori ismi boş bırakılamaz.", "Hata Kodu: 200.");
            }
            else if (data.result == 300) {
                toastr.error("Görsel eklemeden kayıt yapılamaz.", "Hata Kodu: 300.");
            }
            else if (data.result == 400) {
                toastr.error("Görsel yüklemeden kayıt yapılamaz.", "Hata Kodu: 400.");
            }
            else if (data.result == 500) {
                toastr.error("Beklenmedik bir hata oluştu.", "Hata Kodu: 500.");
            }
            else if (data.result == 600) {
                toastr.error("Aynı isimde kategori kaydedilemez.", "Hata Kodu: 600.");
            }
